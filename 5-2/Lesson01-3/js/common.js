...
$(function(){
	...
	$(document).on('click', '[data-toggle=layer]', function(){
		...
		btn.hasClass('icon-hand') ? btn.toggleClass('off') : btn.toggleClass('on');
	...
	$('body').on('click','[data-toggle=layerClse]',function(event){
		var t = $(this);
		var root = t.hasClass('icon-layer-clse') ? t.parent() : t;
		var btn = $('[data-toggle="layer"]').filter('[data-target="#'+root.attr('id')+'"]');

		root.hide().closest('.layer-wrp').removeClass('open');
